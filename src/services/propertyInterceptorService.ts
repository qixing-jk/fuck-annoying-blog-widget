                    ...
                    for (const rule of ruleList) {
                        if (activeConfig[rule.featureName] && rule.setter) {
                            if (rule.setter.call(this, value) === true) {
                                console.log(`[Property Interceptor] Setter for "${propertyName}" blocked by feature "${rule.featureName}".`);
                                return; // 成功拦截，直接返回
                            }
                        }
                    }
                ...
                get: function () {
                    ...
                    for (const rule of ruleList) {
                        if (activeConfig[rule.featureName] && rule.getter) {
                            ...
                            if (result !== undefined) {
                                console.log(`[Property Interceptor] Getter for "${propertyName}" intercepted by feature "${rule.featureName}".`);
